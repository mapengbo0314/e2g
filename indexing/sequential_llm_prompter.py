...
class DryRunConversation:
    ...
    def prompt(self, user_prompt: str) -> str:
        ...
        if key in self.dry_run_map:
            return self.dry_run_map[key]
        
        # Fallback to a structured mock based on schema
        return self._build_mock_response(user_prompt)

    def _build_mock_response(self, user_prompt: str) -> str:
        # We reuse the logic from _SimpleConversation or similar
        # but here we'll just return a generic valid JSON for the schema
        mock_obj = _SimpleConversation(
            system_prompt=self.system_prompt,
            agent_name=self.agent_name,
            output_schema_type=self.output_schema_type,
        )._build_default_output(user_prompt)
...
class GeminiLlmPrompter(LlmPrompter):
    ...
    def _create_single_conversation(
        ...
        if self._config.use_vertex_ai and self._config.vertex_ai_project_id:
            try:
                ...
                logging.warning("Failed to initialize Vertex AI conversation: %e", e)

        # 2. Try Google AI Studio (API Key)
        if self._config.google_api_key:
            try:
                ...
                logging.warning("Failed to initialize Google AI conversation: %e", e)

        # 3. Fallback to Mock (preserving original behavior for non-API environments)
    ...
    def prompt_for_indexing(
        ...
        if verifier:
            ...
            # We use the system_prompt's directory_contents as source context for verification
            # In a real system, this would be the raw code.
