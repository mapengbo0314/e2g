...
ROOT_MAP_FILE_PATTERN = r"root_map_v(\d+)\.md"
LLM_INDEX_FILE_PATTERN = r"llm_index_v(\d+)\.md"
...
def get_rootmap_file_name(epoch: int) -> str:
    """Returns the root map file name for the given epoch."""
    return f"root_map_v{epoch}.md"
...
class LocalState(State):
    ...
    # Helper Methods
    def _get_rootmap_path(self, epoch: int) -> Path:
        """Gets the full path to the root map file without checking existence."""
        return self._state_dir / get_rootmap_file_name(epoch)

    def _get_path(self, path: str, epoch: int) -> Path:
        ...
        if (
            ...
            try:
                ...
                pass
        return self._state_dir / path_obj / get_index_file_name(epoch)
