...
3. **WORKFLOW ENFORCEMENT**: You must orchestrate tasks through the strict lifecycle defined in `{{HARNESS_DIR}}/rules/unified_superpower_workflow.md`. This lifecycle is ALWAYS ON and must be followed: Brainstorming -> Planning -> TDD -> Implementation -> Review -> Verification.
...
To delegate to any of the following specialized subagents, you MUST invoke them via your platform's native subagent tool (e.g., `@<agent_name>` in Gemini CLI, or the `Task` tool in Claude Code):
