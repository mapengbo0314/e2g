...
Architecturally, the directory is organized into specialized components that collaborate to provide this automation. The service subdirectory contains the core business logic implemented as BoO Actions, which leverage a suite of shared utilities in common for interacting with repository, executing CodeMaker schematics for template-based code generation, and formatting files via fmtserver.
...
1. **Input Validation and Transformation**: Parameters from the proto request are often converted into standardized system identifiers using the Container' utility class, ensuring naming consistency across repository, Rapid, and L-Space.
...
3. **repository Automation**: The output from CodeMaker is passed to the 'repositoryService", which automates the creation of a Cit workspace and a corresponding repository changelist (CL). The service automatically adds reviewers, sets specific CL tags (like DO_NOT_SUBMIT or
...
*com.google.gmsplatform.onboarding.common.repositoryService': An abstraction layer for repository operations, allowing the service to create changes from CodeMaker output, format CL files, mail changes for review, and manage CL tags programmatically
...
- repository API: Accessed via "repositoryService" to automate the creation and management of Monorepo changelists.
...
"FakerepositoryService" is available in "common/testing for tests that require a stateful, predictable simulation of repository operations. Integration tests for shell-based tools (like mppcli) utilize
...
* common/repositoryService.java
* common/repositoryServicelmpl.java
* common/README.md
...
* common/testing/FakerepositoryService.java
...
Architectural Patterns and Gotchas
The codebase follows an orchestration agent pattern where BoQ Actions serve as high-level controllers that drive a sequence of external system mutations (repository, Rapid, L-Space, Ga
* Key Individual Components
...
service/: Contains the core business logic of the onboarding service implemented as BoQ Actions. This directory orchestrates multi-step workflows across systems like repository, Ra common/*: Provides shared utilities and abstraction layers used by the service actions. Key components include "repositoryService' for managing changelists, "Codemakertetper' for ex client/: The auto-generated Boclient library for the PlatformOnboardingService'. It allows other services within Google to consume the onboarding automation RPCs by includ1
...
# Key Interfaces
*gmsplatform.onboarding.proto.PlatformOnboardingService': The primary RPC entry point for automating GMS Platform container and module onboarding. It provides high-level operati com.google.gmsplatform.onboarding.common.repositoryService: An internal abstraction for repository operations. It provides methods to create changelists from CodeMaker output, read/writ
